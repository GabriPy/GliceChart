    ...
    <div class="card-body p-0">
      <div class="p-4 pb-2 flex items-center justify-between">
        <div class="flex items-center gap-2">
          <div class="p-1.5 bg-base-300 rounded-lg">
            <i class="fa-solid fa-clock w-3 h-3 opacity-50 text-[12px]"></i>
          </div>
          <span class="text-[9px] uppercase font-black tracking-widest opacity-40">{{ $t('insulin.historyTitle') }}</span>
        </div>
      ...
      <div class="max-h-[180px] overflow-y-auto px-3 pb-4 scrollbar-hide">
        <div v-if="!store.insulinRecords.length" class="text-center py-6 opacity-20 flex flex-col items-center gap-1">
          <i class="fa-solid fa-box-archive w-6 h-6 text-[24px]"></i>
          ...
          <div v-for="ins in sortedInsulin" :key="ins.id"
            ...
            <div v-if="editingId !== ins.id" class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <div class="w-1 h-8 rounded-full" :class="ins.type === 'rapid' ? 'bg-primary' : 'bg-secondary'"></div>
              ...
              <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                <button @click="startEdit(ins)"
                  ...
                  <i class="fa-solid fa-pencil text-[10px] mt-0.5"></i>
                </button>
                <button @click="store.removeInsulin(ins.id)"
                  ...
                  <i class="fa-solid fa-trash text-[10px] mt-0.5"></i>
                </button>
              </div>
            </div>
            ...
            <div v-else class="flex flex-col gap-2 p-1">
              <div class="flex items-center justify-between px-1">
                <span class="text-[8px] font-black uppercase opacity-40">{{ $t('insulin.editTitle') }}</span>
                <span class="text-[10px] font-black text-primary">{{ formatUnits(editForm.units) }}{{ $t('common.unitSymbol') }}</span>
                <button @click="editingId = null"
                  class="btn btn-ghost btn-xs h-4 min-h-0 text-[8px] uppercase">{{ $t('common.cancel') }}</button>
              </div>

              <div class="grid grid-cols-2 gap-1">
                <button @click="editForm.type = 'rapid'" class="btn btn-xs rounded-lg text-[8px] uppercase font-black"
                  :class="editForm.type === 'rapid' ? 'btn-primary' : 'btn-outline opacity-50'">{{ $t('home.rapid') }}</button>
                <button @click="editForm.type = 'slow'" class="btn btn-xs rounded-lg text-[8px] uppercase font-black"
                  :class="editForm.type === 'slow' ? 'btn-secondary' : 'btn-outline opacity-50'">{{ $t('home.slow') }}</button>
              </div>

              <div class="flex items-center gap-2">
                <input type="number" v-model.number="editForm.units" step="0.5" lang="en-US"
              ...
              <button @click="saveEdit" class="btn btn-xs btn-primary w-full rounded-lg text-[8px] font-black uppercase"
