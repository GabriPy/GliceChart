<template>
  <aside :class="collapsed ? 'w-16' : 'w-64'" class="h-full bg-base-200 border-r border-base-content/5 flex flex-col z-[100] transition-all duration-200 overflow-hidden">
    <!-- Logo + Collapse -->
    <div :class="collapsed ? 'px-2 py-2 mb-2 flex items-center justify-center' : 'p-4 mb-2 flex items-center justify-between'">
      ...
      <div v-else class="flex items-center justify-center">
        <div class="flex items-center justify-center rounded-2xl border border-primary/20 bg-primary/15 shadow-sm w-10 h-10 p-1.5">
          <img :src="favicon" alt="logo" class="object-contain w-6 h-6" />
        </div>
      </div>
    </div>

    <div v-show="!collapsed" class="px-4 text-[8px] font-black opacity-20 uppercase tracking-[0.2em]">Dashboard {{ APP_VERSION_LABEL }}</div>
    ...
    <div class="p-3 mt-auto border-t border-base-content/5">
      <div class="flex items-center" :class="collapsed ? 'justify-center' : 'justify-between gap-2 mb-3'">
        <button @click="toggle" class="btn btn-ghost btn-sm btn-square" :class="collapsed ? '' : 'mr-2'" title="Comprimi/espandi sidebar">
        ...
        <div class="flex items-center gap-2" :class="collapsed ? 'justify-center w-full' : ''">
          ...
          <div v-show="!collapsed" class="relative">
            <button ref="themeBtn" @click="toggleThemes" class="btn btn-ghost btn-sm gap-2 normal-case hover:bg-base-200" type="button">
              <Palette class="w-4 h-4" />
              <span class="font-bold text-xs uppercase tracking-widest opacity-60">Tema</span>
            </button>

            <teleport to="body">
              <div v-if="showThemes" ref="themeDropdown" :style="dropdownStyle" class="p-3 shadow-2xl bg-base-300/90 backdrop-blur-xl rounded-box w-64 border border-white/10">
                <ul class="menu">
                  <li v-for="t in themes" :key="t" class="mb-1 last:mb-0">
                    <button class="flex items-center justify-between p-2 rounded-xl outline-none focus:outline-none transition-all duration-200 bg-base-100/50 hover:bg-base-200 w-full" :class="{ 'ring-2 ring-primary ring-offset-2 ring-offset-base-300': currentThemeLocal === t }" @click="applyTheme(t)" :data-theme="t">
...
const currentThemeLocal = store.theme
