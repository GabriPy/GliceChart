...
app.post('/api/auth/verify', async (req, res) => {
  ...
  try {
    ...
    if (hashPin(normalizedPin) !== hash) return res.status(401).json({ error: 'PIN errato' });
...
async function handleRecoveryVerify(req, res) {
  const { code } = req.body || {};
...
app.post('/api/auth/recovery/verify', handleRecoveryVerify);
app.post('/api/auth/recovery-verify', handleRecoveryVerify);
